...
#
# # START METADATA
#   recommendation = ced7804d
#   function = fed_ensure_selinux_installed
#   applicable =
# # END METADATA
#
#
...
fed_ensure_selinux_installed()
{
	...
	fed_ensure_selinux_installed_fix()
	{
		...
		echo -e "- Installing SELinux" | tee -a "$LOG" 2>> "$ELOG"
		$G_PM libselinux
		echo -e "- End remediation - Ensure SELinux is installed" | tee -a "$LOG" 2>> "$ELOG"
	}
